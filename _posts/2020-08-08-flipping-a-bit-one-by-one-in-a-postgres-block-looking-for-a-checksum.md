---
layout: post
title: Flipping a bit one by one in a postgres block looking for a checksum
...
TOAST stands for "The Oversized-Attribute Storage Technique" and allows to broke up large fields value into multiple physical rows (see the [PostgreSQL documentation](https://www.postgresql.org/docs/current/storage-toast.html) for more details).
...
### Retrieve the chunk\_id for each tuple directly from the user table
...
<img src="{{ site.baseurl }}/assets/images/toasted_attributes.png" class="aligncenter size-full wp-image-3542" width="1100" height="777" />

Note that the toast relation id is also part of the tuple data.

As you can see from the slide we can get the information with pageinspect, so let's use it to build a query to retrieve the chunk\_id and the toast relation id from the tuples:
...
as you can see we are able to get the chunk\_id and the toast relation id from the tuples.
...
and then checking the chunk\_ids from the toast itself:
...
### Retrieve the chunk\_id and chunk\_seq directly from the toast index
...
Note that the chunk\_ids coming from the index, the user table tuples and the toast itself match.

### Use case example: a toast’s page is corrupted and I want to know which tuples from the user table are affected

Say the page 12 of the toast is corrupted, then we could get the affected chunk\_id and chunk\_seq from the toast index that way (by filtering on the ctid):
...
And then look back at the user table tuples that way by filtering on the chunk\_id:
